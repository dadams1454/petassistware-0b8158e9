              ...
              <div className="ml-10 flex items-baseline space-x-4">
                {menuItems.map((item) => (
                  <Link
                    key={item.label}
                    to={item.path}
                    className={`text-foreground hover:bg-secondary hover:text-white px-3 py-2 rounded-md text-sm font-medium ${location.pathname === item.path ? 'bg-secondary text-white' : 'text-foreground'}`}
                  >
                    {item.label}
                  </Link>
                ))}
        ...
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          {menuItems.map((item) => (
            <Link
              key={item.label}
              to={item.path}
              className={`text-foreground hover:bg-secondary hover:text-white block px-3 py-2 rounded-md text-base font-medium ${location.pathname === item.path ? 'bg-secondary text-white' : 'text-gray-600 hover:bg-gray-700 hover:text-white'}`}
              aria-current="page"
            >
              <div className="flex items-center">
                {item.icon}
           ...
           {user ? (
            ...
            <Link to="/auth" className="text-blue-500 hover:text-blue-700 block px-3 py-2 rounded-md text-base font-medium">
              Login
            </Link>
          )}
        </div>
