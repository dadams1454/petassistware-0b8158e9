...
export const getTimeSinceLastDogLetOut = (lastBreakTime: string | null) => {
  if (!lastBreakTime) return 'Never';
  return formatDistanceToNow(parseISO(lastBreakTime), { addSuffix: true });
};

const DogLetOutList: React.FC<DogLetOutListProps> = ({ dogs, onObservationClick }) => {
  return (
        ...
        <div key={dog.dog_id} className="flex items-center bg-white dark:bg-slate-800 rounded-md p-2 shadow-sm">
          ...
          <div className="flex-grow min-w-0">
            ...
            <div className="flex items-center text-xs text-muted-foreground">
              ...
              <span className="truncate">{getTimeSinceLastDogLetOut(lastBreak)}</span>
            </div>
          </div>
