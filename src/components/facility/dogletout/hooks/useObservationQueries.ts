...
export const useObservationQueries = (observations: ObservationsMap) => {
  ...
  const hasObservation = useCallback((dogId: string, timeSlot: string = '', activeCategory: string = 'dogletout') => {
  ...
  const getObservationDetails = useCallback((dogId: string, activeCategory: string = 'dogletout') => {
    // Determine which category to filter by
    const targetCategory = activeCategory === 'feeding' ? 'feeding_observation' : 'observation';
