  ...
  const calculateGrowthStats = (): GrowthStats => {
    ...
    if (sortedWeights.length > 1) {
      ...
      return {
        ...
        maxGrowthRate: Math.max(...sortedWeights.map(w => w.percent_change || 0)),
        minGrowthRate: Math.min(...sortedWeights.map(w => w.percent_change || 0).filter(p => p !== 0)),
        dailyGrowthAverage: averageGrowth,
        weeklyGrowthAverage: averageGrowth * 7,
        lastWeight: currentWeight,
        firstWeight: oldestRecord.weight,
        totalGain: totalGrowth,
        percentGain: (totalGrowth / oldestRecord.weight) * 100,
        growthRate,
        lastWeekGrowth,
        projectedWeight
      };
